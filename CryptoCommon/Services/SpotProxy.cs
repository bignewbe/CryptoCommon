...
namespace CryptoCommon.Services
{
    ...
    public class SpotProxy : CINotification, ISpotProxy
    {
        ...
        public ConcurrentDictionary<string, SpotOrder> OpenOrders { get; private set; } = new ConcurrentDictionary<string, SpotOrder>();
        public ConcurrentDictionary<string, SpotOrder> ClosedOrders { get; private set; } = new ConcurrentDictionary<string, SpotOrder>();

        //private Dictionary<string, string> _refIdToPrevRefId = new Dictionary<string, string>();
        private ConcurrentQueue<ManualCommand<SpotOrder>> _ordersToSubmit = new ConcurrentQueue<ManualCommand<SpotOrder>>();

        //private ConcurrentQueue<string> _orderIdToUpdateTimeLast = new ConcurrentQueue<string>();
        private ConcurrentQueue<SpotOrder> _orderToCheck = new ConcurrentQueue<SpotOrder>();
        ...
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<SpotOrder> OnOrderOpened;
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<SpotOrder> OnOrderClosed;
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<SpotOrder> OnOrderUpdated;
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<SpotOrder> OnOrderCancelled;
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<List<SpotOrder>> OnOpenOrderListChanged;
        public event PortableCSharpLib.EventHandlers.ItemChangedEventHandler<List<SpotOrder>> OnClosedOrderListChanged;
        ...
        public SpotProxy(ISpotTradeService trade, List<string> symbols, string dumpfile, int timerInterval, bool isSimuationMode, bool isEnableLog)
        {
            ...
            //this.OnOpenOrderListChanged += (object sender, IList<SpotOrder> orders) => this.LogDebug($"order  added: {order.RefId} {OpenOrders[orderId].OrderId} {OpenOrders[orderId].Symbol}");
        }
        ...
        public List<SpotOrder> GetOpenOrders(string symbol)
        {
            ...
            //if (orders != null)
            //{
            //    var lst = orders.ConvertAll(o => new SpotOrder(o));
            //    return lst;
            //}
            //return null;
        }

        public List<SpotOrder> GetClosedOrders(string symbol)
        {
        ...
        private void RemoveRefId(SpotOrder order)
        {
            if (_syncOrderAction.ContainsKey(order.Symbol))
        ...
        public void UpdateOrders(params SpotOrder[] orders)
        {
            if (!this.IsStarted) return;
        ...
        //private void FillExistingOrderInfo(SpotOrder o)
        //{
        ...
        /// <param name="isTriggerEvent"></param>
        public void UpdateLastTimeAndCheckOrderLocal(SpotOrder order, bool isTriggerEvent)
        {
            if (order != null)
            {
                var isStopOrder = order.Ordertype == OrderType.stop_buy || order.Ordertype == OrderType.stop_sell;
        ...
        private bool CheckOrderLocal(SpotOrder o, bool isTriggerEvent)
        {
            ...
            lock (this)
            {
                ...
                else if (isExistingOpen && !isExistingClose && !isOrderOpen)
                {
                    ...
                    if (OpenOrders.TryRemove(o.OrderId, out order))
                    {
                        ...
                        //if (isTriggerEvent) this.OnOpenOrderListChanged?.Invoke(this, OpenOrders.Values.ToList().ConvertAll(t => new SpotOrder(t)));
                    ...
                    if (ClosedOrders.TryAdd(o.OrderId, o))
                    {
                        ...
                        //if (isTriggerEvent) this.OnClosedOrderListChanged?.Invoke(this, ClosedOrders.Values.ToList().ConvertAll(t => new SpotOrder(t)));
                ...
                else if (!isExistingOpen && isExistingClose && isOrderOpen)
                {
                    ...
                    if (OpenOrders.TryAdd(o.OrderId, o))
                    {
                        if (isTriggerEvent)
                        {
                            ...
                            //this.OnOpenOrderListChanged?.Invoke(this, OpenOrders.Values.ToList().ConvertAll(t => new SpotOrder(t)));
                ...
                else if (!isExistingOpen && !isExistingClose && !isOrderOpen)
                {
                    ...
                    if (ClosedOrders.TryAdd(o.OrderId, o))
                    {
                        if (o.RefId == "SWPHAUSDT36001048eabx10" || o.RefId == "SWPHAUSDT36001048eabx7")
                            this.LogDebug($"existing open order closed: {ConvertOrderToStr(o)}");
        ...
        void ProcessOrdersTosubmitQueue()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    ManualCommand<SpotOrder> command;
        ...
        public async Task Initialize()
        {
            ...
            try
            {
                ...
                if (this.OpenOrders.Count > 0)
                    this.OnOpenOrderListChanged?.Invoke(this, OpenOrders.Values.ToList().ConvertAll(o => new SpotOrder(o)));

                if (this.ClosedOrders.Count > 0)
                    this.OnClosedOrderListChanged?.Invoke(this, ClosedOrders.Values.ToList().ConvertAll(o => new SpotOrder(o)));
            ...
            lock (this)
            {
                try
                {
                    if (File.Exists(this._fileNameDump))
                    {
                        ...
                        this.OpenOrders = JsonConvert.DeserializeObject<ConcurrentDictionary<string, SpotOrder>>(items[0]);
                        this.ClosedOrders = JsonConvert.DeserializeObject<ConcurrentDictionary<string, SpotOrder>>(items[1]);

                        var orders = this.ClosedOrders.Values.Where(o => o.DealAmount > 0 || o.State != OrderState.cancelled).ToList();
                        this.ClosedOrders = new ConcurrentDictionary<string, SpotOrder>(orders.ToDictionary(o => o.OrderId, o => o));
        ...
        public static string ConvertOrderToStr(SpotOrder o)
        {
